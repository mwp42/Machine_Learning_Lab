...
#as.factor(Order.Month) to keep month as discrete levels (no "month 12.5"" plotted)
```

```{r}
month_cat <- orders %>%
  group_by(Order.Month, Category)%>%   #keep Category in data for visualizaiton below
  summarise(monthly_quantity=sum(Quantity))
...
#as.factor(Year) to keep month as discrete levels (no continuous gradient for "year" plotted)
...
#no need to filter for 'Returned == "Yes"' because then Num_of_Returns will be 0
...
glm_model <- train(Returned ~ ., 
                   ...
                   metric = "ROC",   #ROC curve
                   maximize = TRUE,
                   trControl = fitControl)
#ASK: how is Generalized Linear Model similar to logistic regression***
...
plot(rocCurve, print.thres = "best")  #0.041 labeled point is the best threshold

# Find the best threshold from the ROC curve
probsTest <- predict(glm_model, data.test, type = "prob")
threshold <- 0.041 #0.041 labeled point is the best threshold from above plot
